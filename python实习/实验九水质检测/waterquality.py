...
# 图像切割及特征提取
DEFAULT_PATH  = '../data/images/'  # 图片所在路径
# 自定义获取图片名称函数
def getImgNames(path=DEFAULT_PATH ):
    '''
    获取指定路径中所有图片的名称
    :param path: 指定的路径
    :return: 名称列表
...
for i in range(n):
    img = Image.open(DEFAULT_PATH+imgNames[i])  # 读取图片
