...
unsigned int h_len:5; // length of the header 
...
struct hostent * hp; 
int bwrote,bread,datasize,times; 
...
//程序运行输打印的第一条语句， icmp header长度为12 
printf("the length of icmp header is %d\n",sizeof(IcmpHeader));

//开辟空间，icmp_data，char型指针指向该内存空间的首地址 
icmp_data = (char*)xmalloc(sizeof(IcmpHeader) + MAX_PACKET); 

//开辟空间， recvbuf，char型指针指向该内存空间的首地址，存储接收数据 
recvbuf = (char*)xmalloc(sizeof(IcmpHeader) + MAX_PACKET + 20); 
...
int i;
     ...
     for(i=0;i<times;++i){ 
         ...
         //sendto()函数使用方法及返回值参考实验指导 
         bwrote = sendto(sockRaw,icmp_data,datasize,0,(struct sockaddr*)&dest,sizeof(dest)); 
         printf("the length of bwrote is %d\n",bwrote);
         ...
         bread = recvfrom(sockRaw,recvbuf,MAX_PACKET + sizeof(IcmpHeader) + 20,0,(struct sockaddr*)&from,&fromlen); 
         printf("the length of bread is %d\n",bread);
...
iphdr = (IpHeader *)buf; 
printf(" iphdr->h_len = %d. ",iphdr->h_len); 
iphdrlen = (iphdr->h_len) * 4 ; // number of 32-bit words *4 = bytes 
printf(" iphdrlen = %d. ",iphdrlen); 
